...
# Write all that in a .env for the other scripts
ENV_FILE=/mnt"$HOST_MODULE_DIR"/.env
cat > "$ENV_FILE" <<EOF
# Generated by install.sh for host $HOST
DISK=$DISK
PART_PREFIX=$PART_PREFIX
SWAP_SIZE=$SWAP_SIZE
ROOT_PART=$PRIMARY_PART
HOST=$HOST
DOTFILES=$DOTFILES
EOF
echo "Environment file written: $ENV_FILE"
ln -s $ENV_FILE /mnt"$DOTFILES"/modules/scripts/host.env
